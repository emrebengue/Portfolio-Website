...
const ProjectList = ({ index, name, description, tags, image, link }) => {
  return (
    <motion.div variants={fadeIn("up", "spring", index * 0.5, 0.75)}>
      ...
      >
        <div className="relative w-full h-[230px]">
          ...
          />
          <div className="absolute top-0 right-0 flex justify-end m-1 card-img_hover">
            <div
    ...
    <>
      ...
      <div className="mt-20 flex flex-wrap gap-7 justify-center items-center">
        {projects.map((project, index) => (
          <ProjectList key={`project-${index}`} {...project} />
        ))}
      </div>
    </>
  );
